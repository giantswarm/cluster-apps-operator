...
func Test_ClusterNoProxy(t *testing.T) {
	podCidrConfig := podcidr.Config{InstallationCIDR: "10.0.0.0/16"}
	podCidr, err := podcidr.New(podCidrConfig)
	if err != nil {
		t.Fatal(err)
	...
	for _, configMap := range configmaps {
		if strings.HasSuffix(configMap.Name, "-cluster-values") {
			...
			assertEquals(t, "cluster.local,100.64.0.0/13,100.96.0.0/11,svc,127.0.0.1,localhost", cmData.Cluster.Proxy.NoProxy, "Wrong noProxy list")
		}
	}
}

func Test_ClusterNoProxyWithGlobalNoProxy(t *testing.T) {
	podCidrConfig := podcidr.Config{InstallationCIDR: "10.0.0.0/16"}
	podCidr, err := podcidr.New(podCidrConfig)
	if err != nil {
		t.Fatal(err)
	...
	for _, configMap := range configmaps {
		if strings.HasSuffix(configMap.Name, "-cluster-values") {
			...
			assertEquals(t, "cluster.local,100.64.0.0/13,100.96.0.0/11,corp.intranet.com,svc,127.0.0.1,localhost", cmData.Cluster.Proxy.NoProxy, "Wrong noProxy list")
		}
	}
}

func Test_ClusterNoProxyWithEmptyCIDRs(t *testing.T) {
	podCidrConfig := podcidr.Config{InstallationCIDR: "10.0.0.0/16"}
	podCidr, err := podcidr.New(podCidrConfig)
	if err != nil {
		t.Fatal(err)
	...
	config := Config{
		...
		RegistryDomain: "quay.io/giantswarm",
		Proxy: proxy.Proxy{
			HttpProxy:  "http://user:password@192.168.12.12:3128",
			HttpsProxy: "http://user:password@192.168.12.12:3128",
		},
	}
	resource, err := New(config)
	if err != nil {
	...
	for _, configMap := range configmaps {
		if strings.HasSuffix(configMap.Name, "-cluster-values") {
			...
			assertEquals(t, "cluster.local,svc,127.0.0.1,localhost", cmData.Cluster.Proxy.NoProxy, "Wrong noProxy list")
			assertEquals(t, "http://user:password@192.168.12.12:3128", cmData.Cluster.Proxy.HttpProxy, "Wrong httpProxy")
			assertEquals(t, "http://user:password@192.168.12.12:3128", cmData.Cluster.Proxy.HttpsProxy, "Wrong httpsProxy")
		}
	}
}
